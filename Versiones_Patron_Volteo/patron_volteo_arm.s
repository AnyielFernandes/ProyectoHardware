...
patron_volteo_arm
	...
	ADD R8,R13,#40		;Momentaneamente R8=@ de parámetros pasadas por pila
	LDMIA R8,{R8-R10}	;R8=SF, R9=SC y R10=color
	
	ADD R6,R6,R8		;FA=FA+SF
	AND R6,R6,#0xFF		;Convertimos el número en uint8
	ADD R7,R7,R9		;CA=CA+SC
	AND R7,R7,#0xFF		;Convertimos el número en uint8
	...
	MOVEQ R0,#0			;Patrón no encontrado
	BEQ fin
...
fin_while
	...
	LSRSEQ R11,R11,#31	;miramos si el bit de signo es 0 (positivo -> longitud>0)
	MOVEQ R0,#1			;Patrón encontrado
	MOVNE R0,#0			;Patrón no encontrado
